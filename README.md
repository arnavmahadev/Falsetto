...
> **Status:** full architecture implemented end-to-end — both papers' models (AudioCAT,
> FX-Segment, Segment Transformer, Fusion Segment Transformer), the data pipeline,
> training loops, evaluation/analysis, inference and a demo. Verified with a real MERT
> run and a ~60-test suite. Training on the full datasets is left to run on a GPU box;
> the reference MERT extractor and the whole pipeline are confirmed working.
